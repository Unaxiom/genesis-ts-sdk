"use strict";
// @generated by protoc-gen-es v1.3.0 with parameter "target=ts"
// @generated from file users.proto (package Genesis, syntax proto3)
/* eslint-disable */
// @ts-nocheck
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
exports.UsersServiceRegisterMobileDeviceRequest = exports.UsersServiceSearchAllReq = exports.UsersServiceFilterReq = exports.UsersServiceEntityPaginationReq = exports.UsersServicePaginationResponse = exports.UsersServicePaginationReq = exports.UsersList = exports.UserPrimaryInfo = exports.User = exports.UsersServiceUpdateRequest = exports.UsersServiceCreateRequest = exports.USER_SORT_KEY = void 0;
const protobuf_1 = require("@bufbuild/protobuf");
const forms_pb_js_1 = require("./forms_pb.js");
const base_pb_js_1 = require("./base_pb.js");
/**
 *
 * Describes the available sort keys
 *
 * @generated from enum Genesis.USER_SORT_KEY
 */
var USER_SORT_KEY;
(function (USER_SORT_KEY) ***REMOVED***
    /**
     * Fetch ordered results by id
     *
     * @generated from enum value: USER_SORT_KEY_ID_UNSPECIFIED = 0;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_ID_UNSPECIFIED"] = 0] = "USER_SORT_KEY_ID_UNSPECIFIED";
    /**
     * Fetch ordered results by the creation timestamp
     *
     * @generated from enum value: USER_SORT_KEY_CREATED_AT = 1;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_CREATED_AT"] = 1] = "USER_SORT_KEY_CREATED_AT";
    /**
     * Fetch ordered results by the modified timestamp
     *
     * @generated from enum value: USER_SORT_KEY_MODIFIED_AT = 2;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_MODIFIED_AT"] = 2] = "USER_SORT_KEY_MODIFIED_AT";
    /**
     * Fetch ordered results by the approved on timestamp
     *
     * @generated from enum value: USER_SORT_KEY_APPROVED_ON = 3;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_APPROVED_ON"] = 3] = "USER_SORT_KEY_APPROVED_ON";
    /**
     * Fetch ordered results by the approved by field
     *
     * @generated from enum value: USER_SORT_KEY_APPROVED_BY = 4;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_APPROVED_BY"] = 4] = "USER_SORT_KEY_APPROVED_BY";
    /**
     * Fetch ordered results by the approver's role ID
     *
     * @generated from enum value: USER_SORT_KEY_APPROVER_ROLE_ID = 5;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_APPROVER_ROLE_ID"] = 5] = "USER_SORT_KEY_APPROVER_ROLE_ID";
    /**
     * Fetch ordered results by the username
     *
     * @generated from enum value: USER_SORT_KEY_USERNAME = 10;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_USERNAME"] = 10] = "USER_SORT_KEY_USERNAME";
    /**
     * Fetch ordered results by the name
     *
     * @generated from enum value: USER_SORT_KEY_NAME = 11;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_NAME"] = 11] = "USER_SORT_KEY_NAME";
    /**
     * Fetch ordered results by the code
     *
     * @generated from enum value: USER_SORT_KEY_CODE = 12;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_CODE"] = 12] = "USER_SORT_KEY_CODE";
    /**
     * Fetch ordered results by the email address
     *
     * @generated from enum value: USER_SORT_KEY_EMAIL = 13;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_EMAIL"] = 13] = "USER_SORT_KEY_EMAIL";
    /**
     * Fetch ordered results by the phone number
     *
     * @generated from enum value: USER_SORT_KEY_PHONE = 14;
     */
    USER_SORT_KEY[USER_SORT_KEY["USER_SORT_KEY_PHONE"] = 14] = "USER_SORT_KEY_PHONE";
***REMOVED***)(USER_SORT_KEY || (exports.USER_SORT_KEY = USER_SORT_KEY = ***REMOVED******REMOVED***));
// Retrieve enum metadata with: proto3.getEnumType(USER_SORT_KEY)
protobuf_1.proto3.util.setEnumType(USER_SORT_KEY, "Genesis.USER_SORT_KEY", [
    ***REMOVED*** no: 0, name: "USER_SORT_KEY_ID_UNSPECIFIED" ***REMOVED***,
    ***REMOVED*** no: 1, name: "USER_SORT_KEY_CREATED_AT" ***REMOVED***,
    ***REMOVED*** no: 2, name: "USER_SORT_KEY_MODIFIED_AT" ***REMOVED***,
    ***REMOVED*** no: 3, name: "USER_SORT_KEY_APPROVED_ON" ***REMOVED***,
    ***REMOVED*** no: 4, name: "USER_SORT_KEY_APPROVED_BY" ***REMOVED***,
    ***REMOVED*** no: 5, name: "USER_SORT_KEY_APPROVER_ROLE_ID" ***REMOVED***,
    ***REMOVED*** no: 10, name: "USER_SORT_KEY_USERNAME" ***REMOVED***,
    ***REMOVED*** no: 11, name: "USER_SORT_KEY_NAME" ***REMOVED***,
    ***REMOVED*** no: 12, name: "USER_SORT_KEY_CODE" ***REMOVED***,
    ***REMOVED*** no: 13, name: "USER_SORT_KEY_EMAIL" ***REMOVED***,
    ***REMOVED*** no: 14, name: "USER_SORT_KEY_PHONE" ***REMOVED***,
]);
/**
 *
 * Describes the parameters necessary to create a record
 *
 * @generated from message Genesis.UsersServiceCreateRequest
 */
class UsersServiceCreateRequest extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * Stores a globally unique entity UUID. This will be set at the organization level
         *
         * @generated from field: string entity_uuid = 1;
         */
        this.entityUuid = "";
        /**
         * Stores any comment that the user might add during this operation
         *
         * @generated from field: string user_comment = 2;
         */
        this.userComment = "";
        /**
         * The associated vault folder ID
         *
         * @generated from field: int64 vault_folder_id = 9;
         */
        this.vaultFolderId = protobuf_1.protoInt64.zero;
        /**
         * The username of the user
         *
         * @generated from field: string username = 10;
         */
        this.username = "";
        /**
         * The unique employee code by which the user is classified
         *
         * @generated from field: string code = 11;
         */
        this.code = "";
        /**
         * The name of the user
         *
         * @generated from field: string name = 12;
         */
        this.name = "";
        /**
         * The plain text password using which the user can login
         *
         * @generated from field: string plain_text_password = 13;
         */
        this.plainTextPassword = "";
        /**
         * The associated role ID
         *
         * @generated from field: int64 role_id = 14;
         */
        this.roleId = protobuf_1.protoInt64.zero;
        /**
         * The associated mobile role ID
         *
         * @generated from field: int64 mobile_role_id = 15;
         */
        this.mobileRoleId = protobuf_1.protoInt64.zero;
        /**
         * The primary email of the user
         *
         * @generated from field: string email = 16;
         */
        this.email = "";
        /**
         * The optional work email of the user
         *
         * @generated from field: string work_email = 17;
         */
        this.workEmail = "";
        /**
         * The primary contact number of the user
         *
         * @generated from field: string phone = 18;
         */
        this.phone = "";
        /**
         * The birthday of the user
         *
         * @generated from field: string birthday = 30;
         */
        this.birthday = "";
        /**
         * The joining date of the user
         *
         * @generated from field: string joining_date = 31;
         */
        this.joiningDate = "";
        /**
         * The address of the user
         *
         * @generated from field: string address = 32;
         */
        this.address = "";
        /**
         * The city of residence
         *
         * @generated from field: string city = 33;
         */
        this.city = "";
        /**
         * The state of residence
         *
         * @generated from field: string state = 34;
         */
        this.state = "";
        /**
         * The country of residence
         *
         * @generated from field: string country = 35;
         */
        this.country = "";
        /**
         * THe PIN Code of residence
         *
         * @generated from field: string pin_code = 36;
         */
        this.pinCode = "";
        /**
         * THe Blood Group of the user
         *
         * @generated from field: string blood_group = 37;
         */
        this.bloodGroup = "";
        /**
         * The associated shift group ID
         *
         * @generated from field: int64 shift_group_id = 50;
         */
        this.shiftGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material of the user's attendance record
         *
         * @generated from field: int64 attendance_uom_id = 51;
         */
        this.attendanceUomId = protobuf_1.protoInt64.zero;
        /**
         * The associated department (can be 0 to allow seamless transition)
         *
         * @generated from field: int64 department_id = 52;
         */
        this.departmentId = protobuf_1.protoInt64.zero;
        /**
         * The associated payroll group ID of the user
         *
         * @generated from field: int64 payroll_group_id = 53;
         */
        this.payrollGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated tax group ID using which the user's payroll needs to be calculated
         *
         * @generated from field: int64 payroll_tax_group_id = 54;
         */
        this.payrollTaxGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated currency ID of the user's payroll
         *
         * @generated from field: int64 payroll_currency_id = 55;
         */
        this.payrollCurrencyId = protobuf_1.protoInt64.zero;
        /**
         * The basic pay amount of the user (in cents)
         *
         * @generated from field: int64 basic_pay_amount = 56;
         */
        this.basicPayAmount = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material for storing the basic pay amount
         *
         * @generated from field: int64 basic_pay_uom_id = 57;
         */
        this.basicPayUomId = protobuf_1.protoInt64.zero;
        /**
         * The list of dynamic forms
         *
         * @generated from field: repeated Genesis.FormFieldDatumCreateRequest form_data = 70;
         */
        this.formData = [];
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceCreateRequest().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceCreateRequest().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceCreateRequest().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceCreateRequest, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceCreateRequest = UsersServiceCreateRequest;
UsersServiceCreateRequest.runtime = protobuf_1.proto3;
UsersServiceCreateRequest.typeName = "Genesis.UsersServiceCreateRequest";
UsersServiceCreateRequest.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "user_comment", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 9, name: "vault_folder_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 10, name: "username", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 11, name: "code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "name", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 13, name: "plain_text_password", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 14, name: "role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 15, name: "mobile_role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 16, name: "email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 17, name: "work_email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 18, name: "phone", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 30, name: "birthday", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 31, name: "joining_date", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 32, name: "address", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 33, name: "city", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 34, name: "state", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 35, name: "country", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 36, name: "pin_code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 37, name: "blood_group", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 50, name: "shift_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 51, name: "attendance_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 52, name: "department_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 53, name: "payroll_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 54, name: "payroll_tax_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 55, name: "payroll_currency_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 56, name: "basic_pay_amount", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 57, name: "basic_pay_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 70, name: "form_data", kind: "message", T: forms_pb_js_1.FormFieldDatumCreateRequest, repeated: true ***REMOVED***,
]);
/**
 *
 * Describes the parameters necessary to update a record
 *
 * @generated from message Genesis.UsersServiceUpdateRequest
 */
class UsersServiceUpdateRequest extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * Stores any comment that the user might add during this operation
         *
         * @generated from field: string user_comment = 1;
         */
        this.userComment = "";
        /**
         * The ID of the record that needs to be updated
         *
         * @generated from field: int64 id = 2;
         */
        this.id = protobuf_1.protoInt64.zero;
        /**
         * Optional boolean value that stores if a notification needs to be sent to users about the update to the record. This is useful when a subsequent operation needs to be performed immediately (such as send to verification after updating the revision)
         *
         * @generated from field: bool notify_users = 3;
         */
        this.notifyUsers = false;
        /**
         * The associated vault folder ID
         *
         * @generated from field: int64 vault_folder_id = 9;
         */
        this.vaultFolderId = protobuf_1.protoInt64.zero;
        /**
         * The unique employee code by which the user is classified
         *
         * @generated from field: string code = 11;
         */
        this.code = "";
        /**
         * The name of the user
         *
         * @generated from field: string name = 12;
         */
        this.name = "";
        /**
         * The associated role ID
         *
         * @generated from field: int64 role_id = 14;
         */
        this.roleId = protobuf_1.protoInt64.zero;
        /**
         * The associated mobile role ID
         *
         * @generated from field: int64 mobile_role_id = 15;
         */
        this.mobileRoleId = protobuf_1.protoInt64.zero;
        /**
         * The primary email of the user
         *
         * @generated from field: string email = 16;
         */
        this.email = "";
        /**
         * The optional work email of the user
         *
         * @generated from field: string work_email = 17;
         */
        this.workEmail = "";
        /**
         * The primary contact number of the user
         *
         * @generated from field: string phone = 18;
         */
        this.phone = "";
        /**
         * The birthday of the user
         *
         * @generated from field: string birthday = 30;
         */
        this.birthday = "";
        /**
         * The joining date of the user
         *
         * @generated from field: string joining_date = 31;
         */
        this.joiningDate = "";
        /**
         * The address of the user
         *
         * @generated from field: string address = 32;
         */
        this.address = "";
        /**
         * The city of residence
         *
         * @generated from field: string city = 33;
         */
        this.city = "";
        /**
         * The state of residence
         *
         * @generated from field: string state = 34;
         */
        this.state = "";
        /**
         * The country of residence
         *
         * @generated from field: string country = 35;
         */
        this.country = "";
        /**
         * THe PIN Code of residence
         *
         * @generated from field: string pin_code = 36;
         */
        this.pinCode = "";
        /**
         * THe Blood Group of the user
         *
         * @generated from field: string blood_group = 37;
         */
        this.bloodGroup = "";
        /**
         * The associated shift group ID
         *
         * @generated from field: int64 shift_group_id = 50;
         */
        this.shiftGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material of the user's attendance record
         *
         * @generated from field: int64 attendance_uom_id = 51;
         */
        this.attendanceUomId = protobuf_1.protoInt64.zero;
        /**
         * The associated department (can be 0 to allow seamless transition)
         *
         * @generated from field: int64 department_id = 52;
         */
        this.departmentId = protobuf_1.protoInt64.zero;
        /**
         * The associated payroll group ID of the user
         *
         * @generated from field: int64 payroll_group_id = 53;
         */
        this.payrollGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated tax group ID using which the user's payroll needs to be calculated
         *
         * @generated from field: int64 payroll_tax_group_id = 54;
         */
        this.payrollTaxGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated currency ID of the user's payroll
         *
         * @generated from field: int64 payroll_currency_id = 55;
         */
        this.payrollCurrencyId = protobuf_1.protoInt64.zero;
        /**
         * The basic pay amount of the user (in cents)
         *
         * @generated from field: int64 basic_pay_amount = 56;
         */
        this.basicPayAmount = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material for storing the basic pay amount
         *
         * @generated from field: int64 basic_pay_uom_id = 57;
         */
        this.basicPayUomId = protobuf_1.protoInt64.zero;
        /**
         * The list of dynamic forms
         *
         * @generated from field: repeated Genesis.FormFieldDatumCreateRequest form_data = 70;
         */
        this.formData = [];
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceUpdateRequest().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceUpdateRequest().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceUpdateRequest().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceUpdateRequest, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceUpdateRequest = UsersServiceUpdateRequest;
UsersServiceUpdateRequest.runtime = protobuf_1.proto3;
UsersServiceUpdateRequest.typeName = "Genesis.UsersServiceUpdateRequest";
UsersServiceUpdateRequest.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "user_comment", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "notify_users", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 9, name: "vault_folder_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 11, name: "code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "name", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 14, name: "role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 15, name: "mobile_role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 16, name: "email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 17, name: "work_email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 18, name: "phone", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 30, name: "birthday", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 31, name: "joining_date", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 32, name: "address", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 33, name: "city", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 34, name: "state", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 35, name: "country", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 36, name: "pin_code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 37, name: "blood_group", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 50, name: "shift_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 51, name: "attendance_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 52, name: "department_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 53, name: "payroll_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 54, name: "payroll_tax_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 55, name: "payroll_currency_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 56, name: "basic_pay_amount", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 57, name: "basic_pay_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 70, name: "form_data", kind: "message", T: forms_pb_js_1.FormFieldDatumCreateRequest, repeated: true ***REMOVED***,
]);
/**
 *
 * Describes the parameters that are part of a standard response
 *
 * @generated from message Genesis.User
 */
class User extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * Stores a globally unique entity UUID. This will be set at the organization level
         *
         * @generated from field: string entity_uuid = 1;
         */
        this.entityUuid = "";
        /**
         * The status of this user
         *
         * @generated from field: Genesis.STANDARD_LIFECYCLE_STATUS status = 4;
         */
        this.status = base_pb_js_1.STANDARD_LIFECYCLE_STATUS.PREVERIFY_UNSPECIFIED;
        /**
         * Stores the logs of every operation performed on this user
         *
         * @generated from field: repeated Genesis.LogbookLogConciseSLC logs = 5;
         */
        this.logs = [];
        /**
         * The associated vault folder ID
         *
         * @generated from field: int64 vault_folder_id = 9;
         */
        this.vaultFolderId = protobuf_1.protoInt64.zero;
        /**
         * The username of the user
         *
         * @generated from field: string username = 10;
         */
        this.username = "";
        /**
         * The unique employee code by which the user is classified
         *
         * @generated from field: string code = 11;
         */
        this.code = "";
        /**
         * The name of the user
         *
         * @generated from field: string name = 12;
         */
        this.name = "";
        /**
         * The associated role ID
         *
         * @generated from field: int64 role_id = 14;
         */
        this.roleId = protobuf_1.protoInt64.zero;
        /**
         * The associated mobile role ID
         *
         * @generated from field: int64 mobile_role_id = 15;
         */
        this.mobileRoleId = protobuf_1.protoInt64.zero;
        /**
         * The primary email of the user
         *
         * @generated from field: string email = 16;
         */
        this.email = "";
        /**
         * The optional work email of the user
         *
         * @generated from field: string work_email = 17;
         */
        this.workEmail = "";
        /**
         * The primary contact number of the user
         *
         * @generated from field: string phone = 18;
         */
        this.phone = "";
        /**
         * The birthday of the user
         *
         * @generated from field: string birthday = 30;
         */
        this.birthday = "";
        /**
         * The joining date of the user
         *
         * @generated from field: string joining_date = 31;
         */
        this.joiningDate = "";
        /**
         * The address of the user
         *
         * @generated from field: string address = 32;
         */
        this.address = "";
        /**
         * The city of residence
         *
         * @generated from field: string city = 33;
         */
        this.city = "";
        /**
         * The state of residence
         *
         * @generated from field: string state = 34;
         */
        this.state = "";
        /**
         * The country of residence
         *
         * @generated from field: string country = 35;
         */
        this.country = "";
        /**
         * THe PIN Code of residence
         *
         * @generated from field: string pin_code = 36;
         */
        this.pinCode = "";
        /**
         * THe Blood Group of the user
         *
         * @generated from field: string blood_group = 37;
         */
        this.bloodGroup = "";
        /**
         * The associated shift group ID
         *
         * @generated from field: int64 shift_group_id = 50;
         */
        this.shiftGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material of the user's attendance record
         *
         * @generated from field: int64 attendance_uom_id = 51;
         */
        this.attendanceUomId = protobuf_1.protoInt64.zero;
        /**
         * The associated department
         *
         * @generated from field: int64 department_id = 52;
         */
        this.departmentId = protobuf_1.protoInt64.zero;
        /**
         * The associated payroll group ID of the user
         *
         * @generated from field: int64 payroll_group_id = 53;
         */
        this.payrollGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated tax group ID using which the user's payroll needs to be calculated
         *
         * @generated from field: int64 payroll_tax_group_id = 54;
         */
        this.payrollTaxGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated currency ID of the user's payroll
         *
         * @generated from field: int64 payroll_currency_id = 55;
         */
        this.payrollCurrencyId = protobuf_1.protoInt64.zero;
        /**
         * The basic pay amount of the user (in cents)
         *
         * @generated from field: int64 basic_pay_amount = 56;
         */
        this.basicPayAmount = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material for storing the basic pay amount
         *
         * @generated from field: int64 basic_pay_uom_id = 57;
         */
        this.basicPayUomId = protobuf_1.protoInt64.zero;
        /**
         * Stores if MFA has been enabled by the user
         *
         * @generated from field: bool mfa_status = 60;
         */
        this.mfaStatus = false;
        /**
         * The list of dynamic forms
         *
         * @generated from field: repeated Genesis.FormFieldDatum form_data = 70;
         */
        this.formData = [];
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new User().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new User().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new User().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(User, a, b);
***REMOVED***
***REMOVED***
exports.User = User;
User.runtime = protobuf_1.proto3;
User.typeName = "Genesis.User";
User.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "metadata", kind: "message", T: base_pb_js_1.EmployeeMetadata ***REMOVED***,
    ***REMOVED*** no: 3, name: "approval_metadata", kind: "message", T: base_pb_js_1.ApprovalMetadata ***REMOVED***,
    ***REMOVED*** no: 4, name: "status", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.STANDARD_LIFECYCLE_STATUS) ***REMOVED***,
    ***REMOVED*** no: 5, name: "logs", kind: "message", T: base_pb_js_1.LogbookLogConciseSLC, repeated: true ***REMOVED***,
    ***REMOVED*** no: 9, name: "vault_folder_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 10, name: "username", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 11, name: "code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "name", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 14, name: "role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 15, name: "mobile_role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 16, name: "email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 17, name: "work_email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 18, name: "phone", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 30, name: "birthday", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 31, name: "joining_date", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 32, name: "address", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 33, name: "city", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 34, name: "state", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 35, name: "country", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 36, name: "pin_code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 37, name: "blood_group", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 50, name: "shift_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 51, name: "attendance_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 52, name: "department_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 53, name: "payroll_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 54, name: "payroll_tax_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 55, name: "payroll_currency_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 56, name: "basic_pay_amount", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 57, name: "basic_pay_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 60, name: "mfa_status", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 70, name: "form_data", kind: "message", T: forms_pb_js_1.FormFieldDatum, repeated: true ***REMOVED***,
]);
/**
 *
 * Describes the message that is used internally to validate user
 *
 * @generated from message Genesis.UserPrimaryInfo
 */
class UserPrimaryInfo extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * The username of the user
         *
         * @generated from field: string username = 10;
         */
        this.username = "";
        /**
         * The name of the user
         *
         * @generated from field: string name = 11;
         */
        this.name = "";
        /**
         * Stores the hashed password
         *
         * @generated from field: bytes password = 12;
         */
        this.password = new Uint8Array(0);
        /**
         * The associated role ID
         *
         * @generated from field: int64 role_id = 13;
         */
        this.roleId = protobuf_1.protoInt64.zero;
        /**
         * The associated mobile role ID
         *
         * @generated from field: int64 mobile_role_id = 14;
         */
        this.mobileRoleId = protobuf_1.protoInt64.zero;
        /**
         * Stores if MFA has been enabled by the user
         *
         * @generated from field: bool mfa_status = 15;
         */
        this.mfaStatus = false;
        /**
         * Stores the MFA secret
         *
         * @generated from field: bytes mfa_secret = 16;
         */
        this.mfaSecret = new Uint8Array(0);
        /**
         * THe Blood Group of the user
         *
         * @generated from field: string blood_group = 20;
         */
        this.bloodGroup = "";
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UserPrimaryInfo().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UserPrimaryInfo().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UserPrimaryInfo().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UserPrimaryInfo, a, b);
***REMOVED***
***REMOVED***
exports.UserPrimaryInfo = UserPrimaryInfo;
UserPrimaryInfo.runtime = protobuf_1.proto3;
UserPrimaryInfo.typeName = "Genesis.UserPrimaryInfo";
UserPrimaryInfo.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 10, name: "username", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 11, name: "name", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "password", kind: "scalar", T: 12 /* ScalarType.BYTES */ ***REMOVED***,
    ***REMOVED*** no: 13, name: "role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 14, name: "mobile_role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 15, name: "mfa_status", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 16, name: "mfa_secret", kind: "scalar", T: 12 /* ScalarType.BYTES */ ***REMOVED***,
    ***REMOVED*** no: 20, name: "blood_group", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
]);
/**
 *
 * Describes the message consisting of the list of records
 *
 * @generated from message Genesis.UsersList
 */
class UsersList extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * List of records
         *
         * @generated from field: repeated Genesis.User list = 1;
         */
        this.list = [];
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersList().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersList().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersList().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersList, a, b);
***REMOVED***
***REMOVED***
exports.UsersList = UsersList;
UsersList.runtime = protobuf_1.proto3;
UsersList.typeName = "Genesis.UsersList";
UsersList.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "list", kind: "message", T: User, repeated: true ***REMOVED***,
]);
/**
 *
 * Describes a pagination request to retrieve records
 *
 * @generated from message Genesis.UsersServicePaginationReq
 */
class UsersServicePaginationReq extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * If true, then returns only active records. If false, then returns only inactive records
         *
         * @generated from field: bool is_active = 1;
         */
        this.isActive = false;
        /**
         * The number of records that need to be sent in the response
         *
         * @generated from field: int64 count = 2;
         */
        this.count = protobuf_1.protoInt64.zero;
        /**
         * The number that need to be offset by before fetching the records
         *
         * @generated from field: int64 offset = 3;
         */
        this.offset = protobuf_1.protoInt64.zero;
        /**
         * The sort order that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.SORT_ORDER sort_order = 4;
         */
        this.sortOrder = base_pb_js_1.SORT_ORDER.ASCENDING_UNSPECIFIED;
        /**
         * The sort key that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.USER_SORT_KEY sort_key = 5;
         */
        this.sortKey = USER_SORT_KEY.USER_SORT_KEY_ID_UNSPECIFIED;
        /**
         * The status of this user
         *
         * @generated from field: Genesis.STANDARD_LIFECYCLE_STATUS status = 6;
         */
        this.status = base_pb_js_1.STANDARD_LIFECYCLE_STATUS.PREVERIFY_UNSPECIFIED;
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServicePaginationReq().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServicePaginationReq().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServicePaginationReq().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServicePaginationReq, a, b);
***REMOVED***
***REMOVED***
exports.UsersServicePaginationReq = UsersServicePaginationReq;
UsersServicePaginationReq.runtime = protobuf_1.proto3;
UsersServicePaginationReq.typeName = "Genesis.UsersServicePaginationReq";
UsersServicePaginationReq.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "is_active", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "count", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "offset", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 4, name: "sort_order", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.SORT_ORDER) ***REMOVED***,
    ***REMOVED*** no: 5, name: "sort_key", kind: "enum", T: protobuf_1.proto3.getEnumType(USER_SORT_KEY) ***REMOVED***,
    ***REMOVED*** no: 6, name: "status", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.STANDARD_LIFECYCLE_STATUS) ***REMOVED***,
]);
/**
 *
 * Describes the response to a pagination request
 *
 * @generated from message Genesis.UsersServicePaginationResponse
 */
class UsersServicePaginationResponse extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * The number of records in this payload
         *
         * @generated from field: int64 count = 1;
         */
        this.count = protobuf_1.protoInt64.zero;
        /**
         * The number that has been offset before fetching the records. This is the same value that has been sent as part of the pagination request
         *
         * @generated from field: int64 offset = 2;
         */
        this.offset = protobuf_1.protoInt64.zero;
        /**
         * The total number of records that are available
         *
         * @generated from field: int64 total = 3;
         */
        this.total = protobuf_1.protoInt64.zero;
        /**
         * The list of records
         *
         * @generated from field: repeated Genesis.User payload = 4;
         */
        this.payload = [];
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServicePaginationResponse().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServicePaginationResponse().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServicePaginationResponse().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServicePaginationResponse, a, b);
***REMOVED***
***REMOVED***
exports.UsersServicePaginationResponse = UsersServicePaginationResponse;
UsersServicePaginationResponse.runtime = protobuf_1.proto3;
UsersServicePaginationResponse.typeName = "Genesis.UsersServicePaginationResponse";
UsersServicePaginationResponse.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "count", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "offset", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "total", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 4, name: "payload", kind: "message", T: User, repeated: true ***REMOVED***,
]);
/**
 *
 * Describes a pagination request to retrieve records associated with an entity UUID
 *
 * @generated from message Genesis.UsersServiceEntityPaginationReq
 */
class UsersServiceEntityPaginationReq extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * If true, then returns only active records. If false, then returns only inactive records
         *
         * @generated from field: bool is_active = 1;
         */
        this.isActive = false;
        /**
         * The number of records that need to be sent in the response
         *
         * @generated from field: int64 count = 2;
         */
        this.count = protobuf_1.protoInt64.zero;
        /**
         * The number that need to be offset by before fetching the records
         *
         * @generated from field: int64 offset = 3;
         */
        this.offset = protobuf_1.protoInt64.zero;
        /**
         * The sort order that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.SORT_ORDER sort_order = 4;
         */
        this.sortOrder = base_pb_js_1.SORT_ORDER.ASCENDING_UNSPECIFIED;
        /**
         * The sort key that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.USER_SORT_KEY sort_key = 5;
         */
        this.sortKey = USER_SORT_KEY.USER_SORT_KEY_ID_UNSPECIFIED;
        /**
         * The entity UUID that is to be used to filter records
         *
         * @generated from field: string entity_uuid = 6;
         */
        this.entityUuid = "";
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceEntityPaginationReq().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceEntityPaginationReq().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceEntityPaginationReq().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceEntityPaginationReq, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceEntityPaginationReq = UsersServiceEntityPaginationReq;
UsersServiceEntityPaginationReq.runtime = protobuf_1.proto3;
UsersServiceEntityPaginationReq.typeName = "Genesis.UsersServiceEntityPaginationReq";
UsersServiceEntityPaginationReq.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "is_active", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "count", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "offset", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 4, name: "sort_order", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.SORT_ORDER) ***REMOVED***,
    ***REMOVED*** no: 5, name: "sort_key", kind: "enum", T: protobuf_1.proto3.getEnumType(USER_SORT_KEY) ***REMOVED***,
    ***REMOVED*** no: 6, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
]);
/**
 *
 * Describes the base request payload of a filter search
 *
 * @generated from message Genesis.UsersServiceFilterReq
 */
class UsersServiceFilterReq extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * If true, then returns only active records. If false, then returns only inactive records
         *
         * @generated from field: bool is_active = 1;
         */
        this.isActive = false;
        /**
         * The number of records that need to be sent in the response. Returns all records if it is set to -1
         *
         * @generated from field: int64 count = 2;
         */
        this.count = protobuf_1.protoInt64.zero;
        /**
         * The number that need to be offset by before fetching the records
         *
         * @generated from field: int64 offset = 3;
         */
        this.offset = protobuf_1.protoInt64.zero;
        /**
         * The sort order that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.SORT_ORDER sort_order = 4;
         */
        this.sortOrder = base_pb_js_1.SORT_ORDER.ASCENDING_UNSPECIFIED;
        /**
         * The sort key that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.USER_SORT_KEY sort_key = 5;
         */
        this.sortKey = USER_SORT_KEY.USER_SORT_KEY_ID_UNSPECIFIED;
        /**
         * The minimum timestamp that needs to be considered to filter by creation
         *
         * @generated from field: int64 creation_timestamp_start = 6;
         */
        this.creationTimestampStart = protobuf_1.protoInt64.zero;
        /**
         * The maximum timestamp that needs to be considered to filter by creation
         *
         * @generated from field: int64 creation_timestamp_end = 7;
         */
        this.creationTimestampEnd = protobuf_1.protoInt64.zero;
        /**
         * The entity UUID that is to be used to filter records
         *
         * @generated from field: string entity_uuid = 8;
         */
        this.entityUuid = "";
        /**
         * The status of this user
         *
         * @generated from field: Genesis.STANDARD_LIFECYCLE_STATUS status = 10;
         */
        this.status = base_pb_js_1.STANDARD_LIFECYCLE_STATUS.PREVERIFY_UNSPECIFIED;
        /**
         * The start range of approved timestamp
         *
         * @generated from field: int64 approved_on_start = 11;
         */
        this.approvedOnStart = protobuf_1.protoInt64.zero;
        /**
         * The end range of approved timestamp
         *
         * @generated from field: int64 approved_on_end = 12;
         */
        this.approvedOnEnd = protobuf_1.protoInt64.zero;
        /**
         * The ID of the approver
         *
         * @generated from field: int64 approved_by_user_id = 13;
         */
        this.approvedByUserId = protobuf_1.protoInt64.zero;
        /**
         * The role ID of the approver
         *
         * @generated from field: int64 approver_role_id = 14;
         */
        this.approverRoleId = protobuf_1.protoInt64.zero;
        /**
         * The username of the user
         *
         * @generated from field: string username = 20;
         */
        this.username = "";
        /**
         * The name of the user
         *
         * @generated from field: string name = 21;
         */
        this.name = "";
        /**
         * The unique code by which the user is classified
         *
         * @generated from field: string code = 22;
         */
        this.code = "";
        /**
         * The primary email of the user
         *
         * @generated from field: string email = 23;
         */
        this.email = "";
        /**
         * The primary contact number of the user
         *
         * @generated from field: string phone = 24;
         */
        this.phone = "";
        /**
         * The associated role ID
         *
         * @generated from field: int64 role_id = 25;
         */
        this.roleId = protobuf_1.protoInt64.zero;
        /**
         * The associated shift group ID
         *
         * @generated from field: int64 shift_group_id = 26;
         */
        this.shiftGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material of the user's attendance record
         *
         * @generated from field: int64 attendance_uom_id = 27;
         */
        this.attendanceUomId = protobuf_1.protoInt64.zero;
        /**
         * The associated department
         *
         * @generated from field: int64 department_id = 28;
         */
        this.departmentId = protobuf_1.protoInt64.zero;
        /**
         * The associated payroll group ID of the user
         *
         * @generated from field: int64 payroll_group_id = 29;
         */
        this.payrollGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated tax group ID using which the user's payroll needs to be calculated
         *
         * @generated from field: int64 payroll_tax_group_id = 30;
         */
        this.payrollTaxGroupId = protobuf_1.protoInt64.zero;
        /**
         * The associated currency ID of the user's payroll
         *
         * @generated from field: int64 payroll_currency_id = 31;
         */
        this.payrollCurrencyId = protobuf_1.protoInt64.zero;
        /**
         * The associated unit of material for storing the basic pay amount
         *
         * @generated from field: int64 basic_pay_uom_id = 32;
         */
        this.basicPayUomId = protobuf_1.protoInt64.zero;
        /**
         * The optional work email of the user
         *
         * @generated from field: string work_email = 33;
         */
        this.workEmail = "";
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceFilterReq().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceFilterReq().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceFilterReq().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceFilterReq, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceFilterReq = UsersServiceFilterReq;
UsersServiceFilterReq.runtime = protobuf_1.proto3;
UsersServiceFilterReq.typeName = "Genesis.UsersServiceFilterReq";
UsersServiceFilterReq.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "is_active", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "count", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "offset", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 4, name: "sort_order", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.SORT_ORDER) ***REMOVED***,
    ***REMOVED*** no: 5, name: "sort_key", kind: "enum", T: protobuf_1.proto3.getEnumType(USER_SORT_KEY) ***REMOVED***,
    ***REMOVED*** no: 6, name: "creation_timestamp_start", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 7, name: "creation_timestamp_end", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 8, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 10, name: "status", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.STANDARD_LIFECYCLE_STATUS) ***REMOVED***,
    ***REMOVED*** no: 11, name: "approved_on_start", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "approved_on_end", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 13, name: "approved_by_user_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 14, name: "approver_role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 20, name: "username", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 21, name: "name", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 22, name: "code", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 23, name: "email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 24, name: "phone", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 25, name: "role_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 26, name: "shift_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 27, name: "attendance_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 28, name: "department_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 29, name: "payroll_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 30, name: "payroll_tax_group_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 31, name: "payroll_currency_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 32, name: "basic_pay_uom_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 33, name: "work_email", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
]);
/**
 *
 * Describes the request payload for performing a generic search operation on records
 *
 * @generated from message Genesis.UsersServiceSearchAllReq
 */
class UsersServiceSearchAllReq extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * If true, then returns only active records. If false, then returns only inactive records
         *
         * @generated from field: bool is_active = 1;
         */
        this.isActive = false;
        /**
         * The number of records that need to be sent in the response. Returns all records if it is set to -1
         *
         * @generated from field: int64 count = 2;
         */
        this.count = protobuf_1.protoInt64.zero;
        /**
         * The number that need to be offset by before fetching the records
         *
         * @generated from field: int64 offset = 3;
         */
        this.offset = protobuf_1.protoInt64.zero;
        /**
         * The sort order that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.SORT_ORDER sort_order = 4;
         */
        this.sortOrder = base_pb_js_1.SORT_ORDER.ASCENDING_UNSPECIFIED;
        /**
         * The sort key that is to be used to fetch the pagination response
         *
         * @generated from field: Genesis.USER_SORT_KEY sort_key = 5;
         */
        this.sortKey = USER_SORT_KEY.USER_SORT_KEY_ID_UNSPECIFIED;
        /**
         * The entity UUID that is to be used to filter records
         *
         * @generated from field: string entity_uuid = 6;
         */
        this.entityUuid = "";
        /**
         * Limit the search space to the given status
         *
         * @generated from field: Genesis.STANDARD_LIFECYCLE_STATUS status = 10;
         */
        this.status = base_pb_js_1.STANDARD_LIFECYCLE_STATUS.PREVERIFY_UNSPECIFIED;
        /**
         * Describes the key with which the search operation needs to be performed
         *
         * @generated from field: string search_key = 11;
         */
        this.searchKey = "";
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceSearchAllReq().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceSearchAllReq().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceSearchAllReq().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceSearchAllReq, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceSearchAllReq = UsersServiceSearchAllReq;
UsersServiceSearchAllReq.runtime = protobuf_1.proto3;
UsersServiceSearchAllReq.typeName = "Genesis.UsersServiceSearchAllReq";
UsersServiceSearchAllReq.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "is_active", kind: "scalar", T: 8 /* ScalarType.BOOL */ ***REMOVED***,
    ***REMOVED*** no: 2, name: "count", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 3, name: "offset", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 4, name: "sort_order", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.SORT_ORDER) ***REMOVED***,
    ***REMOVED*** no: 5, name: "sort_key", kind: "enum", T: protobuf_1.proto3.getEnumType(USER_SORT_KEY) ***REMOVED***,
    ***REMOVED*** no: 6, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 10, name: "status", kind: "enum", T: protobuf_1.proto3.getEnumType(base_pb_js_1.STANDARD_LIFECYCLE_STATUS) ***REMOVED***,
    ***REMOVED*** no: 11, name: "search_key", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
]);
/**
 *
 * Describes the message that is required to register a user's device
 *
 * @generated from message Genesis.UsersServiceRegisterMobileDeviceRequest
 */
class UsersServiceRegisterMobileDeviceRequest extends protobuf_1.Message ***REMOVED***
    constructor(data) ***REMOVED***
        super();
        /**
         * Stores a globally unique entity UUID. This will be set at the organization level
         *
         * @generated from field: string entity_uuid = 1;
         */
        this.entityUuid = "";
        /**
         * The ID of the user who has possession of the device
         *
         * @generated from field: int64 user_id = 11;
         */
        this.userId = protobuf_1.protoInt64.zero;
        /**
         * The operating system of the device
         *
         * @generated from field: string device_os = 12;
         */
        this.deviceOs = "";
        /**
         * The generated FCM Token
         *
         * @generated from field: string device_token = 13;
         */
        this.deviceToken = "";
        protobuf_1.proto3.util.initPartial(data, this);
***REMOVED***
    static fromBinary(bytes, options) ***REMOVED***
        return new UsersServiceRegisterMobileDeviceRequest().fromBinary(bytes, options);
***REMOVED***
    static fromJson(jsonValue, options) ***REMOVED***
        return new UsersServiceRegisterMobileDeviceRequest().fromJson(jsonValue, options);
***REMOVED***
    static fromJsonString(jsonString, options) ***REMOVED***
        return new UsersServiceRegisterMobileDeviceRequest().fromJsonString(jsonString, options);
***REMOVED***
    static equals(a, b) ***REMOVED***
        return protobuf_1.proto3.util.equals(UsersServiceRegisterMobileDeviceRequest, a, b);
***REMOVED***
***REMOVED***
exports.UsersServiceRegisterMobileDeviceRequest = UsersServiceRegisterMobileDeviceRequest;
UsersServiceRegisterMobileDeviceRequest.runtime = protobuf_1.proto3;
UsersServiceRegisterMobileDeviceRequest.typeName = "Genesis.UsersServiceRegisterMobileDeviceRequest";
UsersServiceRegisterMobileDeviceRequest.fields = protobuf_1.proto3.util.newFieldList(() => [
    ***REMOVED*** no: 1, name: "entity_uuid", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 11, name: "user_id", kind: "scalar", T: 3 /* ScalarType.INT64 */ ***REMOVED***,
    ***REMOVED*** no: 12, name: "device_os", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
    ***REMOVED*** no: 13, name: "device_token", kind: "scalar", T: 9 /* ScalarType.STRING */ ***REMOVED***,
]);
